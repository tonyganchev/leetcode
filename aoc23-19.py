...
class Condition:
    ...
    def from_str(cls, s: str):
        op_idx = max(s.find('<'), s.find('>'))
        c = Condition()
        c.op = s[op_idx]
        c.part_category, t = s.split(c.op)
        c.threshold = int(t)
        return c

    def evaluate(self, parts: Dict[str, int]) -> bool:
        if self.part_category in parts:
            if self.op == '<':
                return parts[self.part_category] < self.threshold
            elif self.op == '>':
                return parts[self.part_category] > self.threshold
            assert False
        return False
...
class Pass(Condition):
    def evaluate(self, _: Dict[str, int]) -> bool:
        return True
...
class Rule:
    ...
    def from_str(cls, s: str):
        ...
        parts = s.split(':')
        if len(parts) == 2:
            r.condition = Condition.from_str(parts[0])
            r.target = parts[1]
        else:
            r.condition = Pass()
            r.target = s
    ...
    def evaluate(self, parts: Dict[str, int]):
        if self.condition.evaluate(parts):
            if self.on_match is None:
                return self.target
            else:
                return self.on_match.evaluate(parts)
        else:
            if self.fall_through is None:
                return self.target
            else:
                return self.fall_through.evaluate(parts)
...
class Workflow:
    ...
    def from_str(cls, s: str):
        w = Workflow()
        w.name, args_b = s.split('{')
        w.rules = tuple(Rule.from_str(rs) for rs in args_b[:-1].split(','))
        return w


def parse_workflows(workflows_s):
    workflow_list = (Workflow.from_str(w) for w in workflows_s.splitlines())
    workflows = {w.name: w for w in workflow_list}
    for wf in workflows.values():
        for i, r in enumerate(wf.rules):
            if r.target not in ['A', 'R']:
                r.on_match = workflows[r.target].rules[0]
            r.fall_through = None if i == len(
                wf.rules) - 1 else wf.rules[i + 1]
    return workflows['in']


def parse_parts(parts_s):
    parts = []
    for p in parts_s.splitlines():
        d = {}
        for pc in p[1:-1].split(','):
            pn, pv = pc.split('=')
            d[pn] = int(pv)
        parts.append(d)
    return parts
...
def solve(data: str) -> int:
    ...
    starting_workflow = parse_workflows(workflows_s)

    parts = parse_parts(parts_s)

    results = 0
    for p in parts:
        r = starting_workflow.rules[0].evaluate(p)
        if r == 'A':
            results += sum(p.values())
    return results


def split_per_condition(rule: Rule, part_range: Dict[str, Tuple[int, int]]) -> Tuple[Dict[str, Tuple[int, int]] | None, Dict[str, Tuple[int, int]] | None]:
    if isinstance(rule.condition, Pass):
        return (part_range, None)
    else:
        op = rule.condition.op
        cat = rule.condition.part_category
        t = rule.condition.threshold
        if op == '<':
            if part_range[cat][1] < t:
                return (part_range, None)
            elif part_range[cat][0] >= t:
                return (None, part_range)
            else:
                return (
                    {k: (lo, t - 1) if k == cat else (lo, hi)
                     for k, (lo, hi) in part_range.items()},
                    {k: (t, hi) if k == cat else (lo, hi)
                     for k, (lo, hi) in part_range.items()}
                )
        elif op == '>':
            if part_range[cat][0] > t:
                return (part_range, None)
            elif part_range[cat][1] <= t:
                return (None, part_range)
            else:
                return (
                    {k: (t + 1, hi) if k == cat else (lo, hi)
                     for k, (lo, hi) in part_range.items()},
                    {k: (lo, t) if k == cat else (lo, hi)
                     for k, (lo, hi) in part_range.items()}
                )
        else:
            assert False
...
def passable_through_rule(rule: Rule, part_range: Dict[str, Tuple[int, int]]) -> int:

    matched, falling_through = split_per_condition(rule, part_range)

    result = 0
    if matched is not None:
        if rule.on_match is None:
            if rule.target == 'A':
                result += prod(hi - lo + 1 for lo, hi in matched.values())
        else:
            result += passable_through_rule(rule.on_match, matched)
    if falling_through is not None:
        if rule.fall_through is not None:
            result += passable_through_rule(rule.fall_through, falling_through)
    return result


def solve2(data: str) -> int:
    return passable_through_rule(parse_workflows(data.split('\n\n')[0]).rules[0], {k: (1, 4000) for k in ('x', 'm', 'a', 's')})
...
def run_tests():
    # run_test(solve, [small_vector], 19114)
    # run_test(solve, [official_vector], 373302)
    run_test(solve2, [small_vector], 167409079868000)
    run_test(solve2, ['''in{A}'''], 256000000000000)
    run_test(solve2, ['''in{a<2:A,R}'''], 64000000000)
    run_test(solve2, ['''in{x>1:R,m>1:R,a>1:R,s>1:R,A}'''], 1)
    run_test(solve2, ['''in{x>2:R,m>1:R,a>1:R,s>1:R,A}'''], 2)
    run_test(solve2, [official_vector], 130262715574114)
    # run_test(solve2, [official_vector], 70086216556038)


small_vector = r'''px{a<2006:qkq,m>2090:A,rfg}
pv{a>1716:R,A}
lnx{m>1548:A,A}
...
official_vector = r'''bvh{a>2239:lr,m>657:mqd,x>928:lz,ndn}
