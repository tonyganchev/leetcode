...
def solve(game_data, cubes=(12, 13, 14)):
    max_red, max_green, max_blue = cubes
    max_draw_per_color = {
        'red': max_red,
        'green': max_green,
        'blue': max_blue
    }
    games = game_data.splitlines()
    id_sum = 0
    for gs in games:
        possible = True
        game_id_str, game_draws = gs.split(': ')
        game_id = int(game_id_str.replace('Game ', ''))
        draws = game_draws.split('; ')
        for ds in draws:
            cube_draws = ds.split(', ')
            for cd in cube_draws:
                n, color = cd.split(' ')
                if int(n) > max_draw_per_color[color]:
                    possible = False
                    break
            if not possible:
                break
...
def solve2(game_data):
    ...
    for gs in games:
        ...
        _, game_draws = gs.split(': ')
        draws = game_draws.split('; ')
        for ds in draws:
            cube_draws = ds.split(', ')
            for cd in cube_draws:
                n, color = cd.split(' ')
                powers[color] = max(powers[color], int(n))
        cube_power = 1
...
small_vector = '''Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green'''
large_vector = '''Game 1: 1 red, 5 blue, 10 green; 5 green, 6 blue, 12 red; 4 red, 10 blue, 4 green
...
run_test(method, [large_vector], 2720)

method = solve2
run_test(method, [small_vector], 2286)
run_test(method, [large_vector], 71535)
