...
class TrieNode:
    def __init__(self, prefix: str, prefix_length: int, children: List[object], subst: str | None):
        self.prefix = prefix
    ...
    def match(self, s):
        ...
        for c in self.children:
            m = c.match(s[self.prefix_length:])
            if m is not None and m.prefix_length > best_match.prefix_length:
                best_match = m
        return None if best_match.subst is None else best_match
...
def solve(original_string):
    ...
    for s in ss:
        ...
        while si < len(s):
            ...
            si += m.prefix_length
        print(s, so)
        cst += int(so[0]) * 10 + int(so[-1])
    return cst


# print(solve('''two1nine
# eightwothree
# abcone2threexyz
# xtwone3four
# 4nineeightseven2
