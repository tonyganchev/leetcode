...
static vector<string> split_and_trim(const string& s, char delimiter) {
    vector<string> tokens;
    string token;
    stringstream ss(s);
    ss >> skipws;
    while (getline(ss, token, delimiter)) {
        // Simple trim for the leading space introduced by the Python split logic
...
static int basic_problem(
        const string_view game_data,
        const int max_red = 12,
        const int max_green = 13,
        const int max_blue = 14) {

    unordered_map<string, int> max_draw_per_color{
        {"red", max_red},
        {"green", max_green},
        {"blue", max_blue}
    };

    auto lines = game_data | views::split('\n');

    auto id_sum = 0;

    for (const auto line_item : lines) {
        string_view line(line_item);
        auto possible = true;
        auto colon_pos = line.find(':');
        auto game_id_str = line.substr("Game "s.length(), colon_pos);
        auto game_draws = line.substr(colon_pos + ": "s.length());
        int game_id;
        ispanstream{ game_id_str } >> game_id;
        auto draws = game_draws | views::split(';');
        for (const auto& ds : draws) {
            auto draw{ trim(string_view{ds}) };
            auto cube_draws = draw | views::split(',');
            for (const auto& cd : cube_draws) {
                ispanstream cube_draw { trim(string_view{cd}) };
                cube_draw >> skipws;
                int n;
                string color;
                cube_draw >> n >> color;

                if (n > max_draw_per_color[color]) {
                    possible = false;
                    break;
                }
            }
...
static int advanced_problem(const string_view game_data) {
    int total_power = 0;
    ...
    for (const auto& line_view : lines) {
        ...
        for (const auto& ds : draws) {
            ...
            for (const auto& cd : cube_draws) {
                ...
                string color;
                cube_draw >> n >> color;
                powers[color] = max(powers[color], n);
            }
        }
        total_power += ranges::fold_left(powers | views::values, 1, multiplies());
    }
    return total_power;
}

int main() {
    auto small_vector = R"(Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green)"sv;
    auto large_vector = R"(Game 1: 1 red, 5 blue, 10 green; 5 green, 6 blue, 12 red; 4 red, 10 blue, 4 green
...
Game 100: 6 blue, 9 green; 3 green, 6 blue; 5 blue, 1 red)"sv;
