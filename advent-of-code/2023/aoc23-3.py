...
def pad_schematic(schematic):
    schematic_width = schematic.find('\n')
    empty_line = '.' * (schematic_width + 1)
    return '\n'.join((empty_line, schematic, empty_line)
                     ).replace('\n', '.\n.').split('\n')


def solve(schematic: str) -> int:
    schematic = pad_schematic(schematic)
    engine_parts = []
    first_digit_idx = None
    for jdx, line in enumerate(schematic[:-1]):
        if jdx == 0:
            continue
        for idx in range(1, len(line)):
            if line[idx].isdigit():
                ...
                if line[idx - 1].isdigit():
                    # can be an engine part...
                    engine_part = False
                    for i in range(first_digit_idx - 1, idx + 1):
                        if issymbol(schematic[jdx - 1][i]) or issymbol(schematic[jdx + 1][i]):
                            engine_part = True
                            break
                    engine_part = engine_part or issymbol(
                        line[first_digit_idx - 1]) or issymbol(line[idx])
                    if engine_part:
                        engine_parts.append(int(line[first_digit_idx:idx]))
    # print(engine_parts)
    return sum(engine_parts)

def expand_part(line: str, idx: int) -> int:
    assert idx > 0
    assert idx < len(line) - 1
    assert line[idx].isdigit()
    start_idx = idx
    end_idx = idx + 1
    for i in range(1, len(line)):
        if not line[idx - i].isdigit():
            break
        start_idx -= 1
    for i in range(1, len(line)):
        if not line[idx + i].isdigit():
...
def solve2(schematic: List[str]):
    ...
    for jdx, line in enumerate(schematic[:-1]):
        for idx in range(1, len(line) - 1):
            if line[idx] == '*':
                adjacent_parts = []
                if schematic[jdx - 1][idx].isdigit():
                    adjacent_parts.append(expand_part(schematic[jdx - 1], idx))
                else:
                    if schematic[jdx - 1][idx - 1].isdigit():
                        adjacent_parts.append(expand_part(schematic[jdx - 1], idx - 1))
                    if schematic[jdx - 1][idx + 1].isdigit():
                        adjacent_parts.append(expand_part(schematic[jdx - 1], idx + 1))
                if schematic[jdx + 1][idx].isdigit():
                    adjacent_parts.append(expand_part(schematic[jdx + 1], idx))
                    ...
                    if schematic[jdx + 1][idx + 1].isdigit():
                        adjacent_parts.append(expand_part(schematic[jdx + 1], idx + 1))
                if line[idx - 1].isdigit():
                    adjacent_parts.append(expand_part(line, idx - 1))
                if line[idx + 1].isdigit():
                    adjacent_parts.append(expand_part(line, idx + 1))
                if len(adjacent_parts) == 2:
                    ratios += adjacent_parts[0] * adjacent_parts[1]
    return ratios


small_vector = '''467..114..
...*......
..35..633.
......#...
617*......
...
...$.*....
.664.598..'''
large_vector = '''............................................411.....................363..134.........463.775..........................506...................
...
method = solve
run_test(method, [small_vector], 4361)
run_test(method, [large_vector], 536202)

method = solve2
run_test(method, [small_vector], 467835)
run_test(method, [large_vector], 78272573)
